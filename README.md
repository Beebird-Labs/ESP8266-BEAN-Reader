...
5. Transmits a compact `"LT,1"` / `"LT,0"` payload to a paired ESP32-C6 via ESP-NOW.
...
Set `TEST_MODE_ENABLED = true` in `main.cpp` to transmit synthetic `LT,1` / `LT,0` messages on a 5-second cycle (every 500 ms) without requiring a live BEAN bus. Useful for validating the ESP-NOW link and receiver firmware independently.

## ESP-NOW Packet Format

```
struct_message {
    char type[10];  // "LT,1" = lights on | "LT,0" = lights off
}
```
